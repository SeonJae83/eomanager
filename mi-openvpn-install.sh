...
# - LOCK SYNC 타이머(OK) / ROUTE REPAIR 타이머(사용 안 함)
...
cleanup_input_rules(){ # 해당 IF 코멘트 규칙 제거
  local IFACE="$1"
...
prune_stale() {
  local d="$1"; shift; local -A alive=(); for ep in "$@"; do alive["$ep"]=1; done
  shopt -s nullglob; for lf in "$d"/*.lock; do ep="$(basename "$lf" .lock)"; [[ -n "${alive[$ep]:-}" ]] || rm -f "$lf"; done
}
...
while IFS= read -r -d '' lf; do
  rel="${lf#$LOCK_DIR/}"
  if [[ -n "${active[$rel]:-}" ]]; then touch "$lf"; rm -f "${lf}.gone" 2>/dev/null || true
  else
    mark="${lf}.gone"
    if [[ -f "$mark" ]]; then rm -f "$lf" "$mark" 2>/dev/null || true; rmdir "$(dirname "$lf")" 2>/dev/null || true
    else : >"$mark"; fi
  fi
...
while IFS= read -r -d '' mg; do
  ...
  if [[ -n "${active[$rel]:-}" ]]; then rm -f "$mg" 2>/dev/null || true
  elif [[ ! -f "$lf" ]] && [[ $(($(date +%s) - $(stat -c %Y "$mg"))) -ge 40 ]]; then rm -f "$mg" 2>/dev/null || true; rmdir "$(dirname "$lf")" 2>/dev/null || true
  fi
...
IFNUM=$(sed -n 's/[^0-9]*\([0-9]\+\).*/\1/p' <<<"$IFACE"); MPORT=$((7000 + IFNUM*10))
...
# ====== PRE WRAPPER (ExecStartPre에서 %i 파싱) ======
cat > "$PREWRAP" <<'PWR'
#!/usr/bin/env bash
set -Eeuo pipefail
unit="${1:-}"; [[ -n "$unit" ]] || exit 0
iface="${unit#mi-}"   # "mi-ens34" -> "ens34"
/usr/local/sbin/mi_route_repair.sh --only "$iface" || true
PWR
chmod 755 "$PREWRAP"

# ====== ROUTE REPAIR (INPUT 규칙/프로필 동기화 포함, 타이머 미사용) ======
cat > "$REPAIR" <<'REP'
#!/usr/bin/env bash
set -Eeuo pipefail
...
fix(){
  ...
  if [[ -f "$CONF" ]]; then
    ...
    # --- profiles update: 최종 conf.local/port 기준, 모든 .ovpn 대상(해당 포트) ---
        ...
        if awk 'BEGIN{exit 1} /^[ \t]*remote[ \t]+/ {exit 0}' "$f"; then
          awk -v ip="$SRV_IP_CONF" -v port="$PORT" '
            ...
            /^[ \t]*remote[ \t]+/ && !done {
              ...
              print "remote " ip " " port extra; done=1; next
            }
            {print}
          ' "$f" > "$f.tmp" && mv "$f.tmp" "$f"
        else
...
rm -f /etc/systemd/system/openvpn-server@.service.d/mi-pre-repair.conf 2>/dev/null || true
