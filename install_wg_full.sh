...
#!/usr/bin/env bash
set -euo pipefail
NIC="${1:?usage: setup_wg_iface.sh <ensNN> [listen_port]}"
NUM="$(echo "$NIC" | sed 's/[^0-9]//g')"
[[ -n "$NUM" ]] || { echo "NIC must contain number"; exit 1; }
DEFAULT_PORT="$((50000 + NUM))"
REQ_PORT="${2:-}"
...
echo "Add user: ${BASE}/wg-add-user.sh wg-ens34 alice"
echo "Del user: ${BASE}/wg-del-user.sh wg-ens34 alice"
